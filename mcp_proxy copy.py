...
## to do add logic to handle the request sent by open web ui to name the chat
...
class WebMCPClient:
    def __init__(self, config_path: str):
        ...
        logger.info(f"WebMCPClient initialized with config {config_path}")
    ...
    async def initialize_servers(self, server_names: List[str] = None, force_reinit: bool = False) -> bool:
        # initialize or reintialize servers with optional cleanup
        # TO DO tweak this logic for mcp_proxy specific
    ...
    async def run_interaction(self, query: str) -> str:
        ...
        if not self.agent:
            ...
            return "Agent is not initialized. Please check your model and tools."
        ...
        try:
            ...
            if isinstance(result, dict) and "messages" in result:
                ...
                return "Agent did not return any message content."
            
            # Fallback and return raw result
    ...
    async def cleanup_sessions(self, server_names_to_keep: List[str] = None):
        ...
        # TO DO add logic for proxy to send commands as prompt from open web ui to /select-server /deselect server /tools etc etc commands
        ...
        for server_name in servers_to_remove:
            if server_name in self.sessions:
                try:
                    ...
                    logger.info(f"Cleaned up sessions for server: {server_name}")
                except Exception as e:
...
async def stream_chat_completion(user_message: str, request: ChatCompletionRequest) -> AsyncGenerator[str, None]:
    # Continuously stream fake data while LLM is working, them stream real response, this is required as in slow machines the open web ui will forever stuck in loading response UI if response if very slow
    # TO DO improve this logic to instead sending of fake data do something else
    ...
    while not response_task.done():
        ...
        char_count += 1
        await asyncio.sleep(2) # to adjust the filler rate
...
async def get_homepage():
    # serve the main html file # to do remove this 
    try: 
...
async def get_openai_models():
    ...
    models = await client.get_ollama_modles()
    return {
        "object": "list",
        "data": [
            OpenAIModel(
...
async def test_stream():
    ...
    def generate():
        test_message = "Hey, this is a test streaming response!"
        completion_id = f"chatcmpl-{uuid.uuid(4).hex[:8]}"
        created_time = int(time.time())
